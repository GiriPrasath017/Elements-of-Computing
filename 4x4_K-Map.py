...
def minimization(k):
    ...
    mini = []
    combo = []
#To solve the bug of overlaping combinations,to make possible to have best minimization.
    multicombo2 = [] 
    multicombo4 = []

    zero_cnt = 0
    prod = 1
    #To count the times we ve applied a combination of squares in the map.
    appended = 0  
    print_k(k)    
    ...
    for i in range(0, 16):
        ...
        #Works as flag to know the most squares for every midterm we can combine.
        combo_value = 0 
        combo_flag = 0
...
#Calculating  free aces in the map and  position with highest combinable value
...
# Minimization for eight squares combination.
        if (k_max == 8):  
            ...
            for j in range(0, 4):
                ...
                if (map_copy[max_thesis] and map_copy[driver_8[max_thesis][j][0] + max_thesis] and map_copy[
                    ...
                    free_aces -= 8
                    break
# Minimization for two squares combination.
        elif (k_max == 2):  
...
# Minimization for four squares combination.
...
reset_frame.grid(row=1,column=2)
global la
la = StringVar()
...
##################
reset_button = Button(selectionframe,bg="lavender", text="Reset", command=reset)
